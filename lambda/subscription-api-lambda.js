...
exports.handler = async (event, context) => {
    ...
    // CORS headers - comprehensive for all browsers
    const headers = {
    ...
    // Support both API Gateway v1.0 and v2.0 payload formats
    ...
    try {
        ...
        // Strip stage prefix from path if present (e.g., /prod/api/... -> /api/...)
        // Only strip if path doesn't already start with /api
        let routePath = path;
        if (!path.startsWith('/api') && path.includes('/api')) {
            // Path format: /stage/api/... -> extract /api/...
            routePath = path.substring(path.indexOf('/api'));
        }
        console.log(`📍 Cleaned path: ${routePath}`);

        let result;

        if (routePath === '/api/vapid-public-key' && method === 'GET') {
            result = await handleGetVapidKey();
        } else if (routePath === '/api/subscribe' && method === 'POST') {
            result = await handleSubscribe(event);
        } else if (routePath === '/api/unsubscribe' && method === 'POST') {
            result = await handleUnsubscribe(event);
        } else if (routePath === '/api/subscriptions' && method === 'GET') {
            result = await handleGetSubscriptions(event);
        } else {
            result = {
                statusCode: 404,
                body: { error: 'Endpoint not found', receivedPath: path, cleanedPath: routePath }
            };
        }
